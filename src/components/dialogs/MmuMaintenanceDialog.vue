<template>
    <v-dialog v-model="showDialog" width="400" persistent :fullscreen="isMobile">
        <panel
            :title="$t('Panels.MmuPanel.MmuMaintenance')"
            :icon="mdiWrenchCog"
            ...
            <v-card-text>
                <v-row>
                    <v-col>
                        <p class="body-2">{{ $t('Panels.MmuPanel.CommingSoon') }}</p>
                    </v-col>
                </v-row>
            </v-card-text>
            <v-card-actions>
...
export default class MmuMaintenanceDialog extends Mixins(BaseMixin, MmuMixin) {
