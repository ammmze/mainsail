<template>
    <v-list-item>
        <v-btn small @click="showChangeSpoolDialog = true">
            ...
            {{ name }}
            <span v-if="spoolId === null" class="font-italic ml-1">({{ $t('Panels.MmuPanel.NoSpool') }})</span>
            <span v-else class="ml-1">({{ spool?.filament?.name ?? '--' }})</span>
        </v-btn>
        <mmu-change-spool-dialog
...
export default class MmuToolsDropdownItem extends Mixins(BaseMixin) {
