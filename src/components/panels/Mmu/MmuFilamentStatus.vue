...
<svg xmlns="http://www.w3.org/2000/svg"
     ...
     width="50%" height="50%" viewBox="180 15 275 405" fill="none"
     preserveAspectRatio="xMidYMid meet" ref="filStatusSvg">
  <defs>
  ...
  <g :style="'stroke:' + colorOutline + '; fill:' + colorFont + '; stroke-linecap: round; stroke-linejoin: round; stroke-width: 0; font-family: Roboto; font-size: 16;'">
    ...
    <text x="278" y="145">Encoder: {{ encoderPos }}mm</text>
    ...
    <g v-if="hasSyncFeedback()">
      <text x="278" y="240">Sync-Feedback</text>
      <g v-if="syncCompressionTriggered()">
        <use xlink:href="#sync-feedback" transform="translate(210, 200) scale(1.2)"/>
        <use xlink:href="#sync-feedback" transform="translate(210, 270) scale(1.2,-1.2)"/>
      </g>
      <g v-if="syncTensionTriggered()">
        <use xlink:href="#sync-feedback" transform="translate(210, 235) scale(1.2)"/>
        <use xlink:href="#sync-feedback" transform="translate(210, 235) scale(1.2,-1.2)"/>
      </g>
    </g>
  </g>
...
<script lang="ts">
/* PAUL: simplier Component ... import Component from 'vue-class-component' */
...
/* PAUL: Example of watch.. encoderPos = number of characters typed */
    encoderData: string = '';

    get encoderPos(): number {
        return this.encoderData.length;
    }
...
/* PAUL: Parent could listen with...
    @Emit('date-sent') // PAUL: date-sent is the event name
    sendDate() {
        console.log("PAUL: Sending new date")
        return new Date();  // This date will be sent as the payload of the event
    }
  <div>
    <ChildComponent @date-sent="handleDateReceived"/>
  </div>

    handleDateReceived(date: Date) {
...
/* PAUL: Vue lifecycle hooks (created, mounted, updated, ... */
    mounted() {
      console.log("PAUL: mounted()")
    }
    ...
    hasPregateSensor(): boolean {
        return true
        /* return this.has_pregate_sensor */
    }
    hasGearSensor(): boolean {
        return true
        /* return this.has_gear_sensor */
    }
